...
const AureliaPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pt-32 pb-20 px-6 sm:px-12 lg:px-16 xl:px-24 bg-[#0F172A]">
      <div className="max-w-7xl mx-auto">
        <div className="mb-12 flex items-center justify-between">
          <Link to="/" className="inline-flex items-center text-[10px] font-black uppercase tracking-[0.4em] text-slate-600 hover:text-neon-gold transition-colors group">
            <ArrowLeft size={16} className="mr-2 group-hover:-translate-x-1 transition-transform" /> Mainframe
          </Link>
          <div className="flex items-center gap-4 px-4 py-2 bg-royal-950/40 border border-neon-gold/10 rounded-full">
             <div className={`w-1.5 h-1.5 rounded-full ${aureliaActive ? 'bg-neon-gold animate-pulse shadow-[0_0_8px_#E5C78B]' : 'bg-slate-700'}`}></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
           <div className="lg:col-span-5 flex flex-col items-center text-center space-y-12">
              <div className="space-y-6">
                ...
                <h1 className="text-4xl md:text-5xl font-display font-black text-white uppercase tracking-tighter">Presence.</h1>
                <p className="text-slate-400 text-sm font-bold italic max-w-sm mx-auto leading-relaxed">
              ...
              <button
                ...
                className={`w-full max-w-xs py-8 rounded-[2rem] font-black text-[12px] tracking-[0.4em] uppercase transition-all duration-500 shadow-3xl active:scale-95 flex items-center justify-center gap-6 ${
                  aureliaActive 
                    ? 'bg-[#0F172A] border-2 border-slate-600 text-slate-400 hover:bg-slate-800' 
                    : 'bg-white text-black border-2 border-white hover:bg-neon-gold hover:border-neon-gold hover:text-black'
                }`}
              >
           ...
           <div className="lg:col-span-7 h-[650px] flex flex-col">
              <div className="flex-grow bg-black/40 backdrop-blur-3xl border border-white/10 rounded-[3rem] p-10 overflow-hidden flex flex-col shadow-inner relative">
                 ...
                 <div className="flex-grow overflow-y-auto space-y-10 scrollbar-hide pr-4">
                    {aureliaTranscript.length === 0 ? (
                      <div className="h-full flex flex-col items-center justify-center text-center opacity-10 space-y-8 grayscale">
                         ...
                         <p className="text-xl font-black uppercase tracking-[0.5em]">Resonance_Standby...</p>
                      </div>
                    ) : (
                      aureliaTranscript.map((line, i) => (
                        <div key={i} className={`flex flex-col gap-3 animate-in slide-in-from-bottom-2 ${line.role === 'user' ? 'items-end' : 'items-start'}`}>
                           <div className={`text-[9px] font-black uppercase tracking-widest ${line.role === 'user' ? 'text-neon-gold opacity-60' : 'text-white opacity-40'}`}>
                              {line.role === 'user' ? 'Operator' : 'Aurelia'}
                           </div>
                           <div className={`max-w-[85%] px-8 py-6 rounded-3xl text-base font-bold border ${
                             line.role === 'user' ? 'bg-neon-gold/5 border-neon-gold/20 text-white italic' : 'bg-white/5 border-white/5 text-slate-300'
                           }`}>
                             {line.text}
                           </div>
                        </div>
